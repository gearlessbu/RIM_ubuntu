...
#else
	filename = std::filesystem::path(fullfilename).filename().string();
	filepath = std::filesystem::path(fullfilename).parent_path().string() + std::string("/");
	...
	for (size_t shape_idx = 0; shape_idx < tinyobj_shapes.size(); ++shape_idx)
	{
		...
		for (size_t index_idx = 0; index_idx < s.mesh.indices.size(); index_idx++)
		{
			...
			if (it == indexMap.end())
			{
				...
				if (tindex.normal_index >= 0)
					normals.push_back(glm::vec3(tinyobj_attributes.normals[tindex.normal_index * 3 + 0], tinyobj_attributes.normals[tindex.normal_index * 3 + 1], tinyobj_attributes.normals[tindex.normal_index * 3 + 2]));
				if (tindex.texcoord_index >= 0)
					texcoords.push_back(glm::vec2(tinyobj_attributes.texcoords[tindex.texcoord_index * 2 + 0], tinyobj_attributes.texcoords[tindex.texcoord_index * 2 + 1]));
...
int MaterialData::updateGLResources()
{
	...
	for (size_t i = 0; i < materials.size(); ++i)
		materialListGL[i] = materials[i].gl;
...
void MeshData::clearCPU()
{
	...
	materialdata.materials.shrink_to_fit();
	materialdata.setDirty();
}