	...
	for (size_t i = 0; i < lightsArray.size(); ++i)
	{
		...
		for (jsonreader::json::iterator it = l.begin(); it != l.end(); ++it)
		{
			...
			if (key == "type")
			{
				...
				{
					if (it.value() == "directional")
						lgl.type = LightType::LT_DIRECTIONAL;
					if (it.value() == "point")
						lgl.type = LightType::LT_POINT;
					if (it.value() == "spot")
						lgl.type = LightType::LT_SPOT;
					if (it.value() == "sphere")
						lgl.type = LightType::LT_SPHERE;
					if (it.value() == "parallelogram")
						lgl.type = LightType::LT_PARALLELOGRAM;
					if (it.value() == "polygon")
						lgl.type = LightType::LT_POLYGON;
				}
			}
