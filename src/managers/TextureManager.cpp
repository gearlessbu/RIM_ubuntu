...
int TextureManagerInstance::loadTexture(std::string filename, bool generate_mipmap, bool RGBtoRGBA)
{
	...
	if (td.data.m_LayerSize.x == 0 || td.data.m_LayerSize.y == 0)
		goto jpg_load_end;
	...
	datasize = td.data.m_sBytePerPixel * td.data.m_LayerSize.x * td.data.m_LayerSize.y * td.data.m_sNumberOfLayers * td.data.m_sNumberOfFaces;
	...
	scanline_size = td.data.m_LayerSize.x * td.data.m_sBytePerPixel;
	...
	for (unsigned int y = 0; y < td.data.m_LayerSize.y; ++y)
	{
		...
		case ImageFormat::RGBA8_UNORM:
		{
			...
			for (unsigned int x = 0; x < td.data.m_LayerSize.x; ++x)
			{
				...
				if (channelcount >= 1)
					obytes[channelcount * x + 0] = c.rgbRed;
				if (channelcount >= 2)
					obytes[channelcount * x + 1] = c.rgbGreen;
				if (channelcount >= 3)
					obytes[channelcount * x + 2] = c.rgbBlue;
				if (channelcount >= 4)
					obytes[channelcount * x + 3] = addAlpha ? std::numeric_limits<byte>::max() : c.rgbReserved;
			}
		}
		break;
...
int TextureManagerInstance::saveTexture(std::string filename, GLHelpers::TextureObject2D tex)
{
	...
	if (!tex.m_uiOpenGLID || !texdatasize)
...
#ifdef _DEBUG
	if (!hasTexture(name))
		throw(std::runtime_error("Texture manager error: requesting non-existant texture."));
#endif
