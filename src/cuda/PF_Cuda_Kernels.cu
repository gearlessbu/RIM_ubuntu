...
__device__ inline void __debugOutputKernel_PF(const char *txt, float x, int i)
{
...
__device__ __forceinline__ int minI_PF(int x, int y)
{
	return ((x < y) ? x : y);
}

__device__ __forceinline__ int maxI_PF(int x, int y)
{
	return ((x < y) ? y : x);
}

__device__ __forceinline__ int flattenIndex_PF(int xInd, int yInd, int zInd)
{
	return xInd + yInd * gridSizeX_PF + zInd * layerSize_PF;
}

__device__ __forceinline__ int flattenIndexBound_PF(int xInd, int yInd, int zInd)
{
...
__device__ __forceinline__ int getGridCellID_PF(int i, float *posParticles)
{
...
__device__ __forceinline__ int getGridCellID_PF(float x, float y, float z)
{
	// Compute x, y and z indices
	x -= boundXMin_PF;
	x /= cellLength_PF;
...
__device__ __forceinline__ int flattenIndexMAC_PF(int xInd, int yInd, int zInd)
{
	return xInd + yInd * (gridSizeX_PF + 1) + zInd * layerSizeMAC_PF;
}

__device__ __forceinline__ float spike(float r)
{
	if (r >= 0.0 && r <= 1.0)
	{
		return 1.0 - r;
...
__device__ __forceinline__ float linWeight(float dx, float dy, float dz)
{
...
__global__ void sparseCoordinateEvaluationKernel_PF(int rows, int cols, int maxNumEntries, int *indices, float *matEntries, float *xEntries, float *outEntries)
{
...
__forceinline__ __device__ float maxAbs(float a, float b)
{
	if (abs(a) > abs(b))
		return a;
	return b;
...
__forceinline__ __device__ float minAbs(float a, float b)
{
	if (abs(a) > abs(b))
		return b;
	return a;
...
__global__ void computeGridVelocitiesKernel_PF(float *particlePositions, float *particleVelocities, float *intermediateSolidVelocities,
											   ...
											   int *particleList, int *ppcData, bool *gridSolid)
{
	...
	if (x <= gridSizeX_PF && y <= gridSizeY_PF && z <= gridSizeZ_PF)
	{
		...
		if (x <= boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 0] = 0;
		if (x >= gridSizeX_PF - boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 0] = 0;
		if (y <= boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 1] = 0;
		if (y >= gridSizeY_PF - boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 1] = 0;
		if (z <= boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 2] = 0;
		if (z >= gridSizeZ_PF - boundaryEpsLargeFloor_PF)
			gridVelocities[i * 3 + 2] = 0;
#else
		if (x <= 0)
			gridVelocities[i * 3 + 0] = 0;
...
__global__ void makeKeyValuePairsKernel_PF(float *particlePositions, int *particleList, int *cellList)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void makePPCDataKernel_PF(int *cellList, int *pPerCellData)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF)
	{
...
__global__ void computeDivergenceKernel_PF(int *ppcData, float *gridVelocities, float *gridWeights, float *gridDensity, float *gridDivergence)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int ii = i;
...
__global__ void jacobiIterationKernel_PF(int *ppcData, float *gridDivergence, float *gridPressureIn, float *gridPressureOut)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int ii = i;
	int z = ii / layerSize_PF;
...
__global__ void velocityCorrectionKernel_PF(int *ppcData, float *gridVelocities, float *gridPressure, float *gridDensity)
{
	// THIS IS A MAC KERNEL!
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	int ii = i;
	...
	if (x <= gridSizeX_PF && y <= gridSizeY_PF && z <= gridSizeZ_PF)
	{
		...
		if (d > 0)
		{
			...
			if (x > 0 && x < gridSizeX_PF)
				gridVelocities[i * 3 + 0] -= (center - left) * fac;
			if (y > 0 && y < gridSizeY_PF)
				gridVelocities[i * 3 + 1] -= (center - bottom) * fac;
			if (z > 0 && z < gridSizeZ_PF)
				gridVelocities[i * 3 + 2] -= (center - back) * fac;
		}
	}
}
...
__device__
	twoVelos_PF
	evaluateVelocities_PF(float x, float y, float z, float *gridVelocities1, float *gridVelocities2)
{
	x -= boundXMin_PF;
	x /= cellLength_PF;
	int xInd = (int)minI_PF(gridSizeX_PF - 1, maxI_PF(0, floor(x)));
...
__forceinline__ __device__ float clampToGridX_PF(float x, bool largerBd = false)
{
#ifdef PF_LARGE_BOUNDARIES
...
#endif
	if (x < boundXMin_PF + bEps)
		x = boundXMin_PF + bEps;
	if (x > boundXMax_PF - bEps * 2)
		x = boundXMax_PF - bEps * 2;
	return x;
}

__forceinline__ __device__ float clampToGridY_PF(float y, bool largerBd = false)
{
#ifdef PF_LARGE_BOUNDARIES
	float bEps = largerBd ? boundaryEpsLarge_PF : boundaryEps_PF;
#else
	float bEps = boundaryEps_PF;
#endif
	if (y < boundYMin_PF + bEps)
		y = boundYMin_PF + bEps;
	if (y > boundYMax_PF - bEps)
		y = boundYMax_PF - bEps;
	return y;
}

__forceinline__ __device__ float clampToGridZ_PF(float z, bool largerBd = false)
{
#ifdef PF_LARGE_BOUNDARIES
	float bEps = largerBd ? boundaryEpsLarge_PF : boundaryEps_PF;
#else
	float bEps = boundaryEps_PF;
#endif
	if (z < boundZMin_PF + bEps)
		z = boundZMin_PF + bEps;
	if (z > boundZMax_PF - bEps * 2)
		z = boundZMax_PF - bEps * 2;
	return z;
}

__forceinline__ __device__ void preventPenetrationFluid_PF(float &x, float &y, float &z, float &vx, float &vy, float &vz, int *ppcData, int *particleList, float *particlePos, float *particleVelos, bool *gridSolid, float *particleNormals, int i, float *particleColors)
{
	...
	for (int xx = xi - 1; xx <= xi + 1; xx++)
	{
		for (int yy = yi - 1; yy <= yi + 1; yy++)
		{
			...
			{
				if (xx < 0 || xx >= gridSizeX_PF || yy < 0 || yy >= gridSizeY_PF || zz < 0 || zz >= gridSizeZ_PF)
					continue;
				int gridInd = flattenIndex_PF(xx, yy, zz);
...
__forceinline__ __device__ void preventPenetrationSolid_PF(int pId, float &x, float &y, float &z, float &vx, float &vy, float &vz, int *ppcData, int *particleList, float *particlePos, float *particleVelos, float *particleNormals, float *particleColors, float *origPositions)
{
			...
			for (int zz = zi - 1; zz <= zi + 1; zz++)
			{
				...
				for (int partLocInd = 0; partLocInd < numParts; partLocInd++)
				{
					...
					if (i != opId && opId >= numFreeParticles_PF)
					{
						...
						if (onx * onx + ony * ony + onz * onz < 0.1)
							continue;
						...
						if (dx * dx + dy * dy + dz * dz < cellLength_PF * cellLength_PF)
						{
							...
							if (odx * odx + ody * ody + odz * odz < cellLength_PF * cellLength_PF * 4)
								continue;
...
__forceinline__ __device__ void preventPenetrationSolid2_PF(int pId, float &x, float &y, float &z, float &vx, float &vy, float &vz, int *ppcData, int *particleList, float *particlePos, float *particleVelos, float *particleNormals)
{
...
__global__ void updateParticleNormalsKernel_PF(float *particleNormalsOrig, int maxNNZ, int *nnzIndices, float *nnzWeights, int cols, float *subspaceCoords, float *particleNormals)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void transferToFluidParticlesKernel_PF(float *gridVelocities, float *gridVelocitiesOrig, float *particlePositions, float *particleVelocities, float *particleNormals, bool *gridSolid, int *ppcData, int *particleList, float *particleColors)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void transferToFluidParticlesKernelNN_PF(float *gridVelocities, float *gridVelocitiesOrig, float *particlePositions, float *particleVelocities, float *particleNormals, bool *gridSolid, int *ppcData, int *particleList)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numFreeParticles_PF)
	{
...
__global__ void preventPenetrationsSolidKernel_PF(float *particlePositions, float *particleVelocities, float *particleNormals, int *ppcData, int *particleList, float *correctedVelocities, float *particleColors, float *origPositions)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF - numFreeParticles_PF)
	{
...
__global__ void transferToSolidParticlesKernel_PF(float *gridVelocities, float *gridVelocitiesOrig, float *particlePositions, float *particleVelocities, float *particleNormals, bool *gridSolid, int *ppcData, int *particleList)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF - numFreeParticles_PF)
	{
...
__global__ void advectKernel_PF(float *gridVelocities, float *randomVelos, int frameNo, float *particlePositions, float *particleVelocities, float *particleNormals, bool *gridSolid, int *ppcData, int *particleList, float *particleColors, float *origPositions)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF)
	{
...
__global__ void advectKernelNN_PF(float *gridVelocities, float *randomVelos, int frameNo, float *particlePositions, float *particleVelocities, float *particleNormals, bool *gridSolid, int *ppcData, int *particleList)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF)
	{
...
__global__ void setMeshVelocitiesKernel_PF(float *particlePositions, int numMeshParticles, float *newMeshPositions, float *intermediateSolidVelocities)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void boundMeshPositionsKernel_PF(float *particlePositions, int numMeshParticles)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numMeshParticles)
	{
...
__global__ void makeParticleColorsKernel_PF(float *particleVelocities, float *solidParticleNormals, float *particleColors)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i < numParticles_PF)
	{
