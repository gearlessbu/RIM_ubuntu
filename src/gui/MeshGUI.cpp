...
void createTextureFrame(std::string &texname, const std::string &type)
{
	if (texname.empty() || !TextureManager::instance().hasTexture(texname))
		return;
...
bool createMaterialFrame(Material &m)
{
	...
	if (ImGui::DragFloat3("Ambient", glm::value_ptr(m.gl.ambient), 0.01f, 0.0f, 1e12f))
		dirty = true;
	if (ImGui::DragFloat3("Diffuse", glm::value_ptr(m.gl.diffuse), 0.01f, 0.0f, 1e12f))
		dirty = true;
	if (ImGui::DragFloat3("Specular", glm::value_ptr(m.gl.specular), 0.01f, 0.0f, 1e12f))
		dirty = true;
	if (ImGui::DragFloat("Shininess", &m.gl.shininess, 0.1f, 0.0f, 1e12f))
		dirty = true;
	if (ImGui::DragFloat3("Emission", glm::value_ptr(m.gl.emission), 0.01f, 0.0f, 1e12f))
		dirty = true;

	if (ImGui::DragFloat("Metallic", &m.gl.metallic, 0.001f, 0.f, 1.f))
		dirty = true;
	if (ImGui::DragFloat("Roughness", &m.gl.roughness, 0.001f, 0.f, 1.f))
		dirty = true;
	if (ImGui::DragFloat("Sheen", &m.gl.sheen, 0.001f, 0.f, 1.f))
		dirty = true;
	if (ImGui::DragFloat("Transmittance", glm::value_ptr(m.gl.transmittance), 0.01f))
		dirty = true;
...
void MeshGUI::draw(GLFWwindow *window, int order)
{
	...
	if (ImGui::TreeNode("Materials"))
	{
		...
		for (size_t i = 0; i < materialList.size(); i++)
		{
			...
			if (ImGui::TreeNode(nodeName.c_str()))
			{
				if (createMaterialFrame(materialList[i]))
					meshptr->materialdata.setDirty();
				ImGui::TreePop();
			}
		}
		ImGui::TreePop();
